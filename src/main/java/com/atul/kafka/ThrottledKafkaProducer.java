...
public class ThrottledKafkaProducer extends Thread{
	
	private String[] args;
	private static final int CLIENT_COUNT=1;
	public static final RateLimiter limiter = RateLimiter.create(10000);
	
	public ThrottledKafkaProducer(String args[]){
		this.args = args;
	}
	
	public static void main(String[] args) throws Exception {
        if (args.length != 3) {
            System.err.println("USAGE: java " + ThrottledKafkaProducer.class.getName() + " url num_records record_size");
            System.exit(1);
        }
        ...
        ThrottledKafkaProducer ins[] = new  ThrottledKafkaProducer[CLIENT_COUNT];
        for(int i =0;i<ins.length;i++){
        	ins[i] = new ThrottledKafkaProducer(args);
        	ins[i].start();
        }
